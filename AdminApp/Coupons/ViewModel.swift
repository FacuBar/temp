...
class CouponViewModel: ObservableObject {
  ...
  func getCoupon(coupon: PromotionCouponReq) {
    ...
    let url = URL(string: "http://192.168.100.2:8080/promotions/\(coupon.promotionId)/coupons/\(coupon.couponId)")!
    ...
    let dataTask = URLSession.shared.dataTask(with: request) { data, response, error in
      ...
      if response.statusCode >= 200 && response.statusCode < 300 {
        ...
        guard let decoded = try? JSONDecoder().decode(PromotionCoupon.self, from: data) else {
        ...
        self.state = .failed(NSError(domain: url.absoluteString, code: response.statusCode))
        return
      }
    }
    dataTask.resume()
